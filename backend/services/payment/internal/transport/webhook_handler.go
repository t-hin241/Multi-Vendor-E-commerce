...
// WebhookHandler receives the payment provider's webhook deliveries. It is
// registered under /api/webhooks, not /api/payments, and carries no JWT
// auth — providers can't present a buyer's bearer token — so every trust
// decision here comes from the use case verifying the payload's signature.
type WebhookHandler struct {
	payments *usecase.PaymentUseCase
	log      zerolog.Logger
}

func NewWebhookHandler(payments *usecase.PaymentUseCase, log zerolog.Logger) *WebhookHandler {
	return &WebhookHandler{payments: payments, log: log}
}
...
func (h *WebhookHandler) Handle(c *gin.Context) {
	...
	if err := h.payments.ProcessWebhook(c.Request.Context(), body, signature); err != nil {
