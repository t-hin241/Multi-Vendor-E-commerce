...
func (h *VendorAddressHandler) Add(c *gin.Context) {
	...
	address, err := h.addresses.Add(c.Request.Context(), middleware.GetUserID(c), c.Param("vendorId"), req.RecipientName, req.Phone, req.Province, req.District, req.Ward, req.StreetAddress)
	if err != nil {
...
func (h *VendorAddressHandler) Update(c *gin.Context) {
	...
	address, err := h.addresses.Update(c.Request.Context(), middleware.GetUserID(c), c.Param("vendorId"), c.Param("id"), req.RecipientName, req.Phone, req.Province, req.District, req.Ward, req.StreetAddress)
	if err != nil {
		httpresponse.HandleError(c, h.log, err)
		return
	}
...
func (h *VendorAddressHandler) Delete(c *gin.Context) {
	if err := h.addresses.Delete(c.Request.Context(), middleware.GetUserID(c), c.Param("vendorId"), c.Param("id")); err != nil {
		httpresponse.HandleError(c, h.log, err)
		return
	}
...
func (h *VendorAddressHandler) SetDefault(c *gin.Context) {
	if err := h.addresses.SetDefault(c.Request.Context(), middleware.GetUserID(c), c.Param("vendorId"), c.Param("id")); err != nil {
		httpresponse.HandleError(c, h.log, err)
		return
	}
